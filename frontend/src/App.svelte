<script>
  ...
  onMount(fetchPosts);
  const themes = [
    'abyss','acid','aqua','autumn','black','bumblebee','business','caramellatte','cmyk',
    'coffee','corporate','cupcake','cyberpunk','dark','dim','dracula','emerald','fantasy',
    'forest','garden','halloween','lemonade','light','lofi','luxury','night','nord',
...
<div class="flex gap-2">
  ...
  <div class="flex-1 flex join">
    <input
      ...
      on:keydown={(e) => {
        if (e.key === 'Enter') {
      searchActive = !!searchQuery.trim(); filterPosts();
        }
      }}
    />
  ...
  <div class="max-w-3xl mx-auto mt-4 px-2">
    <div class={`card border-2 border-${post.color} bg-base-200/30 backdrop-blur-sm shadow-lg`}>
      <div class="card-body p-2 gap-2">
          ...
          <div class="whitespace-pre-wrap break-words p-2">{@html linkify({post})}</div>
      </div>
    </div>
  </div>
{/each}
